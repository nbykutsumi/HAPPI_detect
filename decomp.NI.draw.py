    ...
    for tag in ltag + ["plain"]:
        for var in ["dNI","NdI","dNdI","dP"]:
            ...
            for iens, ens in enumerate(lens):
                ...
                baseDir, sDir = hd_func.path_dpr(model=model, expr=expr, scen=scen, ens=ens, sthpr=sthpr, tag=tag, iYear=iYear_fut, eYear=eYear_fut, season=season, var="dNI")[0:2]
                sPath = hd_func.path_dpr(model=model, expr=expr, scen=scen, ens=ens, sthpr=sthpr, tag=tag, iYear=iYear_fut, eYear=eYear_fut, season=season, var="dNI")[-1]
    
                a3var[iens] = fromfile(sPath, float32).reshape(ny,nx)

            a2var = a3var.mean(axis=0)
            a2t   = (a3var.mean()-0.0)/( a3var.var(axis=0)/sqrt(nens))
