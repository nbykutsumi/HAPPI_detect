...
for scen in lscen:
    ...
    for Year in lYear:
        for Mon in lMon:
            ...
            sumPath = hd_func.path_sumnum(model=model, expr=expr, scen=scen, ens=ens, sthpr=sthpr, tag="plain", sumnum="sum", Year=Year, Mon=Mon)[-1]
            a1  = fromfile(sumPath, float32).reshape(ny,nx)
            ...
            print "*"*50
            print Year,Mon,a1.sum(), a2.sum(),  a2.sum() - a1.sum()
