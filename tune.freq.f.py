...
def load_clim(prj, model, run, ny, nx, regrid=False):
    ...
    if regrid == True:
        #a2freq = us.upscale(a2freq, pergrid=False)
...
# Happi
#lrate = ["org","090-100","080-100","070-100","100-070"]
lrate = ["org"]
for rate in lrate:
    if rate == "org":
      run = "C20-ALL-001"
...
util.mk_dir(oDir)
filePath = os.path.join(oDir, "tune.fbc.csv")
...
for rate in ["ref"]+lrate:
    figname = oDir + "/freq.fbc.%s.png"%(rate)
    cmap    = "gist_stern_r"
    stitle  = "%s"%(rate)
    Fig.DrawMapSimple(da2dat[rate], LatHap, LonHap, vmax=0.05, cmap=cmap, figname=figname, stitle=stitle)
